...
- **Today:** three attention items, running task, due follow-ups, bed rail, letters rail; actionable rows and All overdue / Waiting on service links.
...
- **My clients:** full-width table, functional filters and Running/Waiting cards beneath. Add a person creates a new isolated mock case and updates Today/search/alerts.
- **Client profile:** context bar at top; boxed header; orange Quick exit, dark Ask, soft-green New case note; summary, contact history, files, referrals and plan progress.
...
- **Client tabs:** Profile, Quick exit modal, Plan, Shelters, Referrals, Notes and Letters all open populated, case-scoped surfaces. Existing top-level Working/Done URLs lead to My clients; the actual flows are `/clients/:id/working` and `/clients/:id/done`.
- **Shelters:** table, combined filters, request-based eligibility cards, excluded reasons, service detail dialog, mock capacity/provenance, discovery review requests and call list. Client-scoped selections persist.
...
- **Done:** three option cards and paper trail in one desktop row; explicit candidate/confirm/cancel states, persisted best fit, callback confirmation and idempotent Add all three to plan. Did/Didn't text reflects actual mock operations.
...
9. Alerts → open the relevant client or Mark all read. Global Search → type a name/service and activate a result with Enter. Escape closes dialogs and returns focus.
...
- `npm run test:mock`: 11 deterministic workflow checks covering case isolation, search/ambiguity, filters, saves/alerts, run/Send/Skip, confirmation before booking, persisted serialization, idempotent plan additions, and shelter constraints.
